...
## Nachtrag (26.09.2026): zweiter Rundgang

Rundgang durch beide Systeme als Demokonto (Mia Sommer) und als Lea, Seite für Seite (Bildschirmfotos per Playwright, Seiten per curl). Dabei nachgezogen:

- **Nachschlagen** (`/nachschlagen`, `App\Content\Fundus`): ein Feld für alles, ein Wort sucht direkt (Titel, Kurztext, "Hilft, wenn", Stichworte), ein Satz geht an die KI, die drei bis fünf Treffer mit Begründung wählt. Themen nach Gruppen (`topics.group`), Vorschau im Fenster mit Fundort im Kurs, Gesperrtes mit Tür (Verkaufsseite aus `programs.settings.sales_url`, sonst `settings.shop.url`), Merken, Teilen (Link, für die Coachin in die 1:1-Gespräche), Meine Suchen (`search_histories`), Mein Archiv. Sammlungen (`sammlungen`): die Coachin wählt Treffer aus, benennt sie, schreibt einen Gruss und schickt sie in die Gespräche oder erzeugt einen Link.
- **Werkzeuge für die Coach-Ausbildung** (`tools`, `/werkzeuge`, Filament "Werkzeuge"): wofür, wann, wann nicht, so geht es, Beispiel, Dauer, Material. Sehen nur Personen mit `memberships.settings.ausbildung` und das Team; im Nachschlagen mit Tür (`settings.ausbildung_url`).
- **Digitaler Assistent** (`/coach/assistent`, `App\Ai\Assistent`): Fragen "wo finde ich was" und Betriebsfragen ("was hat Nicole gebucht?") mit Fakten aus der App (Programme, Zugänge, Kontingent, Termine, Gespräch, Buchungen, Lage) und Orten im Coach-Bereich; die KI kennt die App aus einem festen Text plus `settings.ai.wissen`. Dazu Themen prüfen ("Passt" setzt `finder_profiles.is_checked`), Werkzeuge, Geteiltes.
- **Coachees in der App-Hülle** (`/coachees`, nur Team): Auskunft, Ampel mit "kurz nachfragen", Für dich freigegeben, "x in Begleitung · y weitere Kontakte · z warten", Suche, Sortierung (nächster Termin, zuletzt aktiv, Name), Personenkarten mit Termin, Aufgaben, Sitzungen, zuletzt da, Marken "wartet" und "Test". Wie die alte Coachees-Seite, fürs Handy.
- **Falsch angezeigt, behoben:** Eigene Blade-Ansichten im Coach-Bereich (Dossier, Widgets) hatten keine Tailwind-Hilfsklassen, weil Filament 5 keine mitliefert; jetzt in `public/css/coach.css`. Das Dashboard listete dieselbe geteilte Übung sechsmal (je Antwort), jetzt gebündelt je Person und Einheit (`App\Coach\Neues`). Importierte Kampagnen-Beiträge zeigten `{firstname}` und `{login_link}` als Text; `Post::bodyFor()` ersetzt die Platzhalter. Verwaltende sahen in den Impulsen Entwürfe und Team-Beiträge ohne Hinweis; jetzt mit Marke "Nur Team", "Geplant" oder "Ausgeschaltet".
