...
class TenancyTest extends TestCase
{
    ...
    {
        $t = Tenant::create(['slug' => 'a', 'name' => 'A']);
        $t->domains()->create(['domain' => 'a.test', 'is_primary' => true]);

        $this->get('http://a.test/')->assertOk()->assertSee('A');
    }

    public function test_unbekannte_domain_gibt_404(): void
    {
        config(['tenancy.fallback_slug' => null]);
        $this->get('http://unbekannt.test/')->assertNotFound();
    }

    public function test_daten_sind_getrennt(): void
    {
        Schema::create('probe', function ($t) { $t->id(); $t->foreignId('tenant_id'); $t->string('x'); $t->timestamps(); });
        $model = new class extends Model { use BelongsToTenant; protected $table = 'probe'; protected $guarded = []; };
